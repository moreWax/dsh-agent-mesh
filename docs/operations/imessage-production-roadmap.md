...
Nix portable generated the lock, `nix flake check --no-build` evaluated the x86_64 derivations, and the immutable Matrix asset derivation built successfully. The full package derivation now uses `pnpm_10.fetchDeps` with a deterministic source archive and reaches dependency fetching, but Nix-portable/proot on this host became unstable (allocator abort, then a hung retry) before a fixed-output hash could be finalized. Validate the same derivation under native Nix next; reproducible double-build and arm64 Nix execution remain explicit acceptance gates.
