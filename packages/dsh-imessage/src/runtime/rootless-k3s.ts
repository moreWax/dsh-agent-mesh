...
export interface RootlessK3sOptions { baseDir?: string; manifestPath?: string; runner?: CommandRunner; processManager?: UserProcessManager; platform?: NodeJS.Platform; arch?: string }
...
export class RootlessK3sRuntime implements ClusterRuntime {
  ...
  constructor(private readonly options: RootlessK3sOptions = {}) {
    ...
    this.runner = options.runner ?? runCommand; this.binDir = join(baseDir, 'runtime/bin'); this.downloadsDir = join(baseDir, 'runtime/downloads'); this.dataDir = join(baseDir, 'k3s/data'); this.logDir = join(baseDir, 'k3s/logs'); this.kubeconfig = join(baseDir, 'k3s/kubeconfig.yaml'); this.manager = options.processManager
  }
  ...
  async start(signal?: AbortSignal): Promise<void> { const detected = await this.detect(signal); if (!detected.available) throw new RuntimeError('IMESSAGE_RUNTIME_NOT_CONFIGURED', 'Rootless runtime binaries or host capabilities are missing', 'Run the explicit rootless runtime prepare step', false); await mkdir(this.dataDir, { recursive: true, mode: 0o700 }); await this.processManager().start(); for (let attempt = 0; attempt < 30; attempt++) { if (signal?.aborted) throw signal.reason; try { await this.command(['get', 'nodes'], signal); return } catch { await new Promise(resolve => setTimeout(resolve, 1000)) } }; throw new RuntimeError('IMESSAGE_RUNTIME_UNAVAILABLE', 'Rootless k3s did not become ready', 'Inspect the private k3s log and retry', true) }
  ...
  async status(signal?: AbortSignal): Promise<RuntimeStatus> { const detection = await this.detect(signal); const running = detection.available && await this.processManager().running(); const api = running ? await this.command(['get', 'nodes', '-o', 'name'], signal).then(() => true, () => false) : false; const checks = [...detection.checks, { name: 'process', ok: running, required: true, detail: running ? 'running' : 'stopped' }, { name: 'api', ok: api, required: true, detail: api ? 'ready' : 'unavailable' }]; return { kind: this.kind, health: api ? 'ready' : detection.available ? 'degraded' : 'not-configured', detail: api ? 'Rootless k3s is ready' : 'Rootless k3s needs attention', checks, lastCheckedAt: new Date().toISOString() } }
  ...
  async stop(_signal?: AbortSignal): Promise<void> { await this.manager?.stop() }
  async remove(_signal?: AbortSignal): Promise<void> { await this.stop(); await rm(join(this.options.baseDir ?? join(process.env.HOME ?? '.', '.local/share/dsh-imessage'), 'k3s'), { recursive: true, force: true }) }
