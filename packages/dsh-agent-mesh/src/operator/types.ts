...
export interface AgentMeshConfig { socketPath: string | false; tcpUrl: string; preferSocket: boolean; nodeCredentialRef?: string; /** Reference to the pre-shared enrollment token in ctx.credentials (bootstrap enrollment); unset = interactive device flow. */ nodeEnrollmentCredentialRef?: string; timeoutMs: number; autoStartNode?: boolean; autoBeginEnrollment?: boolean; nodeControlPlane?: string; /** sam-node binary selection: '' = auto (bundled suggestion), else an explicit path. */ nodeBinary?: string; stopNodeOnExit?: boolean; nodeAnnouncePrivate?: boolean; callCapabilityRef?: string }
export interface AgentMeshConfigInput { socketPath?: string | false; tcpUrl?: string; preferSocket?: boolean; /** Reference only; secret values belong in ctx.credentials. */ nodeCredentialRef?: string; /** Reference only; pre-shared enrollment token lives in ctx.credentials. */ nodeEnrollmentCredentialRef?: string; timeoutMs?: number; autoStartNode?: boolean; autoBeginEnrollment?: boolean; nodeControlPlane?: string; /** sam-node binary selection: '' = auto (bundled suggestion), else an explicit path. */ nodeBinary?: string; stopNodeOnExit?: boolean; /** Publish RFC1918/ULA addresses; false on the public hub. */ nodeAnnouncePrivate?: boolean; /** Credential ref for the outgoing-call fleet capability. */ callCapabilityRef?: string }
