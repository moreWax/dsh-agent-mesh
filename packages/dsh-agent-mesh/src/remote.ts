...
declare module "@deepseek-ai/dsh-typert-protocol" { interface TypertRemoteNamespaceMap { agentMeshWeb: {
  ...
  fleetAdminRequests(request: { serviceName?: string; peerId?: string }): Promise<RemoteResult<{ ok: boolean; pending?: { requestId: string; label: string; requestedAt?: string }[]; error?: string }>>
  fleetAdminApprove(request: { requestId: string; serviceName?: string; peerId?: string }, approval: ApprovedAction): Promise<RemoteResult<ActionResult>>
  fleetAdminReject(request: { requestId: string; serviceName?: string; peerId?: string }, approval: ApprovedAction): Promise<RemoteResult<ActionResult>>
  fleetAdminMembers(request: { serviceName?: string; peerId?: string }): Promise<RemoteResult<{ ok: boolean; members?: { id: string; name: string; scopes: string[]; createdAt: string; note?: string }[]; error?: string }>>
  fleetInviteCreate(request: { ttlMs?: number; note?: string; serviceName?: string; peerId?: string }, approval: ApprovedAction): Promise<RemoteResult<{ ok: boolean; code?: string; expiresAt?: number; error?: string }>>
  fleetAdminRevoke(request: { id: string; serviceName?: string; peerId?: string }, approval: ApprovedAction): Promise<RemoteResult<ActionResult>>
...
const serveStatusSchema = z.object({ configured: z.boolean(), target: z.string(), port: z.number(), announceName: z.string(), modelAllowlist: z.array(z.string()), runtimeModel: z.string(), running: z.boolean(), models: z.array(z.string()), rowState: z.string().optional(), rowDetail: z.string().optional(), backends: z.array(z.object({ name: z.string(), url: z.string(), present: z.boolean() })) })
