/**
 * Agent-facing onboarding doc — printed by `sam-mesh skill` and mirrored in
 * docs/agent-skill.md. Agents read this to onboard a machine onto a mesh
 * without a human translating our CLI surface for them.
 */
...
sam-mesh node join --control-plane <hub-url> \
  --bootstrap-token-path ~/sam-join-token              # file, never inline
...
\`\`\`bash
sam-mesh token mint --control-plane <hub-url> \
  --admin-token-path <admin-token-file> \
  [--role sam:role:node] [--ssh user@host] [--qr]
\`\`\`
